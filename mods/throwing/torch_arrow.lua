	...
	if self.lastpos.x~=nil then
		...
		elseif node.name ~= "air" and not string.find(node.name, "trail") then
			...
			if not string.find(node.name, "water") and not string.find(node.name, "lava") and not string.find(node.name, "torch") then
				...
				-- Checking a position
				local area_protected = minetest.is_protected(pos, "throwing:arrow_torch")
				 
				if area_protected then
					print("spawn_protected")
					local toughness = 0.9
					if math.random() < toughness then
						minetest.add_item(self.lastpos, 'default:torch')
					else
						minetest.add_item(self.lastpos, 'default:stick')
					end
				else
